...
public class ReservationPresenter implements ReservationController, Presenter {
    ...
    /**
     * @param searchCourse Studiengänge, Benutzergruppen.
     * @return null if error
     */
    public Category[] getCategory(Locale locale, String searchCourse) {
        Category courseCategory = null;
        Category superCategory = facade.getSuperCategory();
        Category[] categories = superCategory.getCategories();
        for (Category category : categories) {
            if (category.getName(locale).equals(searchCourse)) {
                courseCategory = category;
            }
        }
        if (courseCategory == null) {
            logger.error("there is no : " + searchCourse);
        }
