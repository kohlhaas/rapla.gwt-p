...
import org.rapla.client.cookie.RaplaLoginCookie;
...
public class Rapla implements EntryPoint {
	...
	{
		String address = GWT.getModuleBaseURL() + "../rapla/json/"
				+ RemoteStorage.class.getName();
	...
	public void onModuleLoad() {
		...
		if (RaplaLoginCookie.getCookie().isAlreadyLoggedIn()) {
			goToWizard();
		} else {
			...
			sendButton.addClickHandler(new ClickHandler() {
				...
				public void onClick(ClickEvent event) {
					...
					FutureResult<LoginTokens> login = loginService.login(
							textToServer, password, null);
					login.get(new AsyncCallback<LoginTokens>() {

						public void onSuccess(LoginTokens result) {
							logger.info("Login successfull  ");
							RaplaLoginCookie.getCookie().updateLogin(result);
							((AbstractJsonProxy) service).setAuthThoken(result
									.getAccessToken());
							goToWizard();
						}
	...
	private void goToWizard() {
		...
		GWT.runAsync(new RunAsyncCallback() {
			...
			public void onSuccess() {
				...
				FutureResult<List<ReservationImpl>> reservations = service
						.getReservations(null, null, null, null);
