...
class ProxyCreator {
  ...
  String create(final TreeLogger logger, final GeneratorContext context)
    ...
    try {
      ...
      futureResultClass = typeOracle.getType(FutureResult.class.getName());
...
//      }
		...
		if (!callback.getErasedType().getQualifiedSourceName().equals(
		    futureResultClass.getQualifiedSourceName())) {
		  invalid(logger, "Method " + m.getName() + " requires "
  ...
  private void generateProxyMethod(final TreeLogger logger,
    ...
    w.println(");");
    w.println("return new org.rapla.storage.dbrm.ResultImpl<" + resultType.getQualifiedSourceName()+ ">();");
